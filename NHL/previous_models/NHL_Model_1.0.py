...
def get_shot_data(input_year_02=202202, num_games=1312, start=1):
    ...
    for game_id in range(start, start + num_games):
        game_id = str(input_year_02) + f"{game_id:04}"
...
vect_prev_type = tf.keras.layers.TextVectorization(
    standardize=custom_standardize,
    max_tokens=TYPE_EVENTS,
    output_mode='int',  # To create unique integer indices for each token
    output_sequence_length=SEQ_LEN)

# Adapt, apply and onehot encode the vectorization layer
vect_shot_type.adapt([row[NUM_FEAT] for row in train_in])
vect_prev_type.adapt([row[NUM_FEAT + 1] for row in train_in])

vocab1 = vect_shot_type.get_vocabulary()
vocab2 = vect_prev_type.get_vocabulary()
one_hot_vocab = tf.one_hot(vect_shot_type([row[NUM_FEAT] for row in train_in]), len(vocab1))
one_hot_prev = tf.one_hot(vect_prev_type([row[NUM_FEAT + 1] for row in train_in]), len(vocab2))
one_hot_test = tf.one_hot(vect_shot_type([row[NUM_FEAT] for row in test_in]), len(vocab1))
one_hot_prevtest = tf.one_hot(vect_prev_type([row[NUM_FEAT + 1] for row in test_in]), len(vocab2))
